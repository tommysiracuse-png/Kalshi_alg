    ...
    def status_snapshot(self) -> dict[str, Any]:
        ...
        for worker_id, managed in self._workers.items():
            ...
            workers.append({
                ...
                "heartbeatBuildMs": int(getattr(heartbeat, "heartbeat_build_ms", 0) or 0) if heartbeat else None,
                "diagnosticsDurationMs": int(getattr(heartbeat, "diagnostics_duration_ms", 0) or 0) if heartbeat else None,
                "telemetryFlushMs": int(getattr(heartbeat, "telemetry_flush_ms", 0) or 0) if heartbeat else None,
                "heartbeatPublishWaitMs": int(getattr(heartbeat, "heartbeat_publish_wait_ms", 0) or 0) if heartbeat else None,
                "heartbeatCoalescedCount": int(getattr(heartbeat, "heartbeat_coalesced_count", 0) or 0) if heartbeat else 0,
                "lastRichDiagnosticsAtMs": int(getattr(heartbeat, "last_rich_diagnostics_at_ms", 0) or 0) if heartbeat else None,
