...
PRICE_SCALE = 10_000
COUNT_SCALE = 100
SUMMARY_RETENTION_MS = 48 * 60 * 60 * 1000
...
class PortfolioAnalyticsStore:
    ...
    def record_refresh(
        ...
        with self._connect() as db:
            ...
            db.execute(
                ...
                (
                    snapshot_at_ms,
                    summary.get("availableCashUnits"),
                    summary.get("midpointPositionValueUnits"),
                    summary.get("totalPortfolioValueUnits"),
    ...
    @staticmethod
    def _points(rows: list[sqlite3.Row], column: str, maximum: int = 96) -> list[Dict[str, int]]:
        values = [
    ...
    def summary(self, *, window_ms: int = 86_400_000) -> Dict[str, Any]:
        ...
        if partial:
            warnings.append("24-hour change history is still accumulating")
        summary = current.get("summary") if isinstance(current.get("summary"), dict) else {}
        mapping = {
            "availableCash": ("available_cash_units", summary.get("availableCashUnits")),
            "totalPortfolioValue": ("total_portfolio_value_units", summary.get("totalPortfolioValueUnits")),
        ...
        return {
            ...
            "summary": {
                "availableCashUnits": summary.get("availableCashUnits"),
                "midpointPositionValueUnits": summary.get("midpointPositionValueUnits"),
                "totalPortfolioValueUnits": summary.get("totalPortfolioValueUnits"),
    ...
    def positions(self, *, active_run: Optional[Mapping[str, object]]) -> Dict[str, Any]:
        ...
        with self._connect() as db:
            ...
            fill_counts = {
                str(row["market_id"]): int(row["count"])
                for row in db.execute("SELECT market_id,COUNT(*) AS count FROM fills GROUP BY market_id")
            }
            session_markets = set()
        ...
        for item in items:
            ...
            item["totalFillCount"] = fill_counts.get(market_id, 0)
