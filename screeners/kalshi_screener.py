...
MARKET_SCAN_HARD_LIMIT = 20_000_000


def bounded_market_scan_limit(value: Any) -> int:
    """Return a positive, memory-safe upper bound for one market scan."""
    try:
        requested = int(value)
    except (TypeError, ValueError):
        requested = 0
    if requested <= 0:
        return MARKET_SCAN_HARD_LIMIT
    return min(requested, MARKET_SCAN_HARD_LIMIT)
...
class KalshiPublicClient:
    ...
    def list_markets(
        ...
        *,
        ...
        cursor = None
        max_total = bounded_market_scan_limit(max_total)
        while True:
...
def screen_markets(pub: KalshiPublicClient, settings: Dict[str, Any]) -> pd.DataFrame:
    ...
    try:
        ...
        requested_limit = 0
    effective_limit = bounded_market_scan_limit(requested_limit_value)
    scan_warnings: List[str] = []
    if requested_limit <= 0:
        scan_warnings.append(
            f"Invalid or unbounded market scan limit {requested_limit_value!r}; "
            f"using the memory-safe limit of {effective_limit:,}."
        )
    elif requested_limit > effective_limit:
        scan_warnings.append(
            f"Market scan requested {requested_limit:,} markets; capped at the "
            f"memory-safe limit of {effective_limit:,}."
        )

    markets = pub.list_markets(
        status=settings["status"],
        limit=1000,
        max_total=effective_limit,
        mve_filter=settings["mve_filter"],
    )

    toxic_series: set = set()
    toxic_tickers: set = set()
    if settings.get("markout_filter_enabled"):
        try:
            # Keep telemetry and history paths rooted at the repository even
            # though the screener implementation now lives in ``screeners``.
            workspace_dir = str(Path(__file__).resolve().parent.parent)
            toxic_series, toxic_tickers, series_stats, ticker_stats = markout_history.load_unprofitable(
                workspace_dir=workspace_dir,
                net_threshold_cents=float(settings["markout_filter_net_threshold_cents"]),
                ticker_min_fills=int(settings["markout_filter_ticker_min_fills"]),
                series_min_fills=int(settings["markout_filter_series_min_fills"]),
                horizon_seconds=int(settings["markout_filter_horizon_seconds"]),
                lookback_days=float(settings["markout_filter_lookback_days"]),
                fee_factor=float(settings["markout_filter_fee_factor"]),
                total_net_threshold_cents=float(
                    settings["markout_filter_total_net_threshold_cents"]
                ),
            )
            print(
                markout_history.format_stats_report(
                    series_stats,
                    ticker_stats,
            ...
            print(f"markout_history: filter disabled due to error: {exc}", file=sys.stderr)
            toxic_series = set()
            toxic_tickers = set()
    ...
    for market in markets:
        ...
        if toxic_tickers and ticker_str in toxic_tickers:
            dropped_toxic += 1
            continue
        if toxic_series:
            series_key = markout_history.series_from_ticker(ticker_str)
    ...
    if toxic_series or toxic_tickers:
        print(
            f"markout_history: dropped {dropped_toxic} markets "
            f"(toxic series={sorted(toxic_series)}, toxic tickers={sorted(toxic_tickers)})",
            file=sys.stderr,
        )

    reached_limit = scanned_markets >= effective_limit
    if reached_limit and requested_limit <= effective_limit:
        scan_warnings.append(
            f"Market scan reached its {effective_limit:,}-market limit; "
            "additional markets may not have been evaluated."
        )
    for warning in scan_warnings:
        print(f"screener: {warning}", file=sys.stderr)
    ...
    if df.empty:
        df.attrs["market_scan"] = scan_metadata
        df.attrs["warnings"] = tuple(scan_warnings)
        return df
...
def build_export_dataframe(screen_df: pd.DataFrame, settings: Dict[str, Any]) -> pd.DataFrame:
    if screen_df.empty:
        ...
        export_df = screen_df.head(int(settings["top_n"])).reset_index(drop=True)
    export_df.attrs.update(screen_df.attrs)
    return export_df
...
def build_settings_from_args(args: argparse.Namespace) -> Dict[str, Any]:
    return {
        ...
        # Markout-history toxic-series filter
        "markout_filter_enabled": args.markout_filter_enabled,
        "markout_filter_net_threshold_cents": args.markout_filter_net_threshold_cents,
        "markout_filter_ticker_min_fills": args.markout_filter_ticker_min_fills,
        "markout_filter_series_min_fills": args.markout_filter_series_min_fills,
        "markout_filter_horizon_seconds": args.markout_filter_horizon_seconds,
        "markout_filter_lookback_days": args.markout_filter_lookback_days,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Markout-history toxic-series filter
    parser.add_argument(
        "--markout-filter-enabled",
        action="store_true",
        default=bool(cfg("MARKOUT_FILTER_ENABLED", True)),
        help="Drop screener candidates whose series/ticker has negative realized net P&L.",
    )
    parser.add_argument(
        "--no-markout-filter",
        dest="markout_filter_enabled",
        action="store_false",
        help="Disable the markout-based toxic-series filter.",
    )
    parser.add_argument(
        "--markout-filter-net-threshold-cents",
        type=float,
        default=float(cfg("MARKOUT_FILTER_NET_THRESHOLD_CENTS", -1.0)),
        help="Drop tickers/series with avg net per-fill cents below this value.",
    )
    parser.add_argument(
        "--markout-filter-ticker-min-fills",
        type=int,
        default=int(cfg("MARKOUT_FILTER_TICKER_MIN_FILLS", 3)),
        help="Min fills before a single ticker can be flagged toxic (aggressive).",
    )
    parser.add_argument(
        "--markout-filter-series-min-fills",
        type=int,
        default=int(cfg("MARKOUT_FILTER_SERIES_MIN_FILLS", 20)),
        help="Min fills before an entire series can be flagged toxic (conservative).",
    )
    parser.add_argument(
        "--markout-filter-horizon-seconds",
        type=int,
        default=int(cfg("MARKOUT_FILTER_HORIZON_SECONDS", 5)),
    )
    parser.add_argument(
        "--markout-filter-lookback-days",
        type=float,
        default=float(cfg("MARKOUT_FILTER_LOOKBACK_DAYS", 14.0)),
    )
    parser.add_argument(
        "--markout-filter-fee-factor",
        type=float,
        default=float(cfg("MARKOUT_FILTER_FEE_FACTOR", 0.55)),
        help="Must match V1.py default_fee_factor_for_maker_quotes.",
    )
    parser.add_argument(
        "--markout-filter-total-net-threshold-cents",
        type=float,
        default=float(cfg("MARKOUT_FILTER_TOTAL_NET_THRESHOLD_CENTS", -300.0)),
        help=(
            "Drop tickers/series whose size-weighted total realized net cents "
            "(marked to ground-truth future market mid) is below this. "
            "Default -300c = $3 hard stop."
        ),
    )
