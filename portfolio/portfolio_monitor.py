...
class PortfolioMonitorConfig:
    ...
    max_cached_orders: int = 5_000
    max_cached_fills: int = 10_000
    max_resting_orders: int = 10_000
    max_fills_per_resting_order: int = 1_000


def _bounded_orders(
    orders: Dict[str, AccountOrder],
    limit: int,
    resting_order_ids: set[str],
) -> tuple[Dict[str, AccountOrder], bool]:
    ...
    newest = sorted(
        orders.items(),
        key=lambda pair: (
            pair[0] in resting_order_ids,
            pair[1].created_at_ms is not None,
            pair[1].created_at_ms or -1,
            pair[0],
        ),
        reverse=True,
    )
...
def _bounded_fills(
    ...
    limit: int,
    resting_order_ids: set[str],
) -> tuple[Dict[str, AccountFill], bool]:
    maximum = max(1, int(limit))
    if len(fills) <= maximum:
        return fills, False
    newest = sorted(
        fills.items(),
        key=lambda pair: (
            pair[1].order_id in resting_order_ids,
            pair[1].created_at_ms is not None,
            pair[1].created_at_ms or -1,
            pair[0],
        ),
...
class PortfolioMonitor(BasePortfolio):
    ...
    def _refresh_sync(self, now_ms: int):
        ...
        overlap_ms = 60_000
        order_limit = max(1, int(self.config.max_cached_orders))
        fill_limit = max(1, int(self.config.max_cached_fills))
        resting_limit = max(1, int(self.config.max_resting_orders))
        per_order_fill_limit = max(1, int(self.config.max_fills_per_resting_order))
        warnings: list[str] = []
        balance = self.client.get_account_balance()
        positions = self.client.list_account_positions(AccountPositionQuery(nonzero_only=True))
        resting = self.client.list_account_orders(
            AccountOrderQuery(status="resting", max_results=resting_limit)
        )
        if len(resting) >= resting_limit:
            warnings.append(
                f"Resting orders reached the {resting_limit}-row safety limit; "
                "the open-order list may be truncated"
            )
        ...
        recent_orders = self.client.list_account_orders(
            AccountOrderQuery(min_created_at_ms=order_start, max_results=order_limit)
        )
        recent_fills = self.client.list_account_fills(
            AccountFillQuery(min_created_at_ms=fill_start, max_results=fill_limit)
        )
        if len(recent_orders) >= order_limit:
            warnings.append(
                f"Order history reached the {order_limit}-row safety limit; "
                "portfolio order history is truncated"
            )
        if len(recent_fills) >= fill_limit:
            warnings.append(
                f"Fill history reached the {fill_limit}-row safety limit; "
                "portfolio fill history is truncated"
            )
        ...
        orders, orders_pruned = _bounded_orders(orders, order_limit, resting_order_ids)
        if orders_pruned:
            warnings.append(
                f"Portfolio order cache is limited to {order_limit} rows; older order history was discarded"
            )
        fills = {
            key: value
        ...
        fills, fills_pruned = _bounded_fills(fills, fill_limit, resting_order_ids)
        if fills_pruned:
            warnings.append(
                f"Portfolio fill cache is limited to {fill_limit} rows; older fill history was discarded"
            )

        orders_with_fills = {fill.order_id for fill in fills.values() if fill.order_id}
        truncated_resting_fill_orders = 0
        for order in resting:
            if order.fill_count_units <= 0 or order.order_id in orders_with_fills:
                continue
            order_fills = self.client.list_account_fills(
                AccountFillQuery(order_id=order.order_id, max_results=per_order_fill_limit)
            )
            if len(order_fills) >= per_order_fill_limit:
                truncated_resting_fill_orders += 1
            for fill in order_fills:
                if fill.fill_id:
                    fills[fill.fill_id] = fill
                    if fill.order_id:
                        orders_with_fills.add(fill.order_id)
            fills, newly_pruned = _bounded_fills(fills, fill_limit, resting_order_ids)
            fills_pruned = fills_pruned or newly_pruned
        if truncated_resting_fill_orders:
            warnings.append(
                f"Fill history reached the {per_order_fill_limit}-row per-order safety limit "
                f"for {truncated_resting_fill_orders} resting order(s)"
            )
        if fills_pruned:
            warnings.append(
                f"Portfolio fill cache is limited to {fill_limit} rows; older fill history was discarded"
        ...
        order_times = [
            item.created_at_ms
            for item in chain(recent_orders, resting)
            if item.created_at_ms is not None
        ]
        fill_times = [
            item.created_at_ms
            for item in chain(recent_fills, fills.values())
            if item.created_at_ms is not None
        ]
        return (
            snapshot,
    ...
    def _build_snapshot(
        ...
        for position in positions:
            ...
            net_realized = realized - fees if realized is not None and fees is not None else None
            market_total = (
            ...
            position_rows.append(
                {
                    ...
                    "currentMarketValueUnits": midpoint_value,
                    "realizedPnlUnits": realized,
                    "netRealizedPnlUnits": net_realized,
                    "feesUnits": fees,
