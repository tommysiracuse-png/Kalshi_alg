...
class BotSettings:
    ...
    api_key_id: str = ""  # set via --api-key-id or KALSHI_API_KEY_ID env var
    private_key_path: str = "./privkey.txt"  # set via --private-key or KALSHI_PRIVATE_KEY_PATH env var
...
class ToxicityModel:
    ...
    def estimate_per_contract_units(self, *, side: str, context: MarketContext) -> int:
        ...
        weighted_total = 0.0
        weight_sum = 0.0
        for horizon_ms, weight in ((5_000, 0.65), (30_000, 0.25), (120_000, 0.10)):
        ...
        queue_ahead_units = context.queue_ahead_units_for_side(side) or 0
        queue_risk_ticks = min(2.0, queue_ahead_units / contracts_to_count_units(25))
        estimated_units += adverse_trade_flow * PRICE_UNITS_PER_CENT * 2.0
        estimated_units += adverse_imbalance * PRICE_UNITS_PER_CENT * 1.0
        estimated_units += queue_risk_ticks * self.settings.default_toxicity_cents * (PRICE_UNITS_PER_CENT / 2.0)
...
class TopOfBookBot:
    ...
    def build_side_quote_decision(
        ...
        *,
        ...
        base_toxicity_units = self.toxicity_model.estimate_per_contract_units(side=side, context=context) + pull_toxicity_penalty_units
        ...
        if not side_quote_allowed or market_target_units is None:
            ...
            return SideQuoteDecision(
                ...
                estimated_toxicity_units=base_toxicity_units,
        ...
        for _ in range(max(1, int(self.settings.candidate_price_levels_to_scan))):
            ...
            candidate = {
                ...
                "expected_edge_units": expected_edge_units,
                "score_units": expected_edge_units - int(round(queue_penalty_units * 0.20)),
            }
            if best_candidate is None or candidate["score_units"] > best_candidate["score_units"]:
                best_candidate = candidate
