...
DEFAULT_MAX_CONCURRENT_PROFILERS = 2
DEFAULT_PROFILER_STAGGER_MAX_SECONDS = 10.0
DEFAULT_PROFILER_SLOT_WAIT_SECONDS = 60.0
DEFAULT_PROFILER_TIMEOUT_GRACE_SECONDS = 20.0
DEFAULT_PROFILER_SLOT_DIR = Path(tempfile.gettempdir()) / (
    f"kalshi-watchdog-profiler-slots-{getattr(os, 'getuid', lambda: 0)()}"
)
...
def deterministic_stagger_seconds(
    ticker: str,
    *,
    interval_seconds: float,
    maximum_seconds: float,
) -> float:
    """Return a stable per-ticker initial delay within the configured window."""
    window_seconds = min(max(0.0, float(interval_seconds)), max(0.0, float(maximum_seconds)))
    if window_seconds <= 0:
        return 0.0
    fraction = _stable_hash(str(ticker)) / float(2**64 - 1)
    return fraction * window_seconds


def profiler_timeout_seconds(args: argparse.Namespace) -> float:
    configured = getattr(args, "profiler_timeout_seconds", None)
    if configured is not None:
        return max(1.0, float(configured))
    return max(
        1.0,
        float(getattr(args, "sample_seconds", 0.0)) + DEFAULT_PROFILER_TIMEOUT_GRACE_SECONDS,
    )


def _try_lock_slot_file(handle: TextIO) -> bool:
...
def _release_slot_file(handle: TextIO) -> None:
    try:
        handle.seek(0)
        handle.truncate()
        handle.write("{}")
        ...
        # The advisory lock is released by close even if metadata cleanup fails.
        pass
    if os.name == "nt":
        import msvcrt
...
def acquire_profiler_slot(
    *,
    ...
    """Acquire one process-wide profiler slot using crash-safe advisory locks."""
    slot_count = max(1, int(maximum_concurrency))
    wait_timeout = max(0.0, float(wait_timeout_seconds))
    slot_directory.mkdir(parents=True, exist_ok=True)
    deadline = time.monotonic() + wait_timeout
    first_slot = _stable_hash(str(ticker)) % slot_count

    while True:
        for offset in range(slot_count):
            slot_index = (first_slot + offset) % slot_count
            slot_path = slot_directory / f"slot-{slot_index}.lock"
            handle = slot_path.open("a+", encoding="utf-8")
            try:
                acquired = _try_lock_slot_file(handle)
            except Exception:
                handle.close()
                raise
            if not acquired:
            ...
            try:
                handle.seek(0)
                handle.truncate()
                json.dump({"pid": os.getpid(), "ticker": ticker}, handle)
                handle.flush()
                yield slot_index
            finally:
                try:
        ...
        if remaining_seconds <= 0:
            raise TimeoutError(
                f"no profiler slot available within {wait_timeout:.1f}s "
                f"(limit={slot_count})"
            )
...
def run_profiler_once(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    timeout_seconds = profiler_timeout_seconds(args)
    slot_directory = Path(
        getattr(args, "profiler_slot_dir", DEFAULT_PROFILER_SLOT_DIR)
    ).expanduser().resolve()
    maximum_concurrency = max(
        1,
        int(getattr(args, "profiler_max_concurrency", DEFAULT_MAX_CONCURRENT_PROFILERS)),
    )
    slot_wait_seconds = max(
        0.0,
        float(getattr(args, "profiler_slot_wait_seconds", DEFAULT_PROFILER_SLOT_WAIT_SECONDS)),
    )
    with acquire_profiler_slot(
        ticker=args.ticker,
        slot_directory=slot_directory,
        maximum_concurrency=maximum_concurrency,
        wait_timeout_seconds=slot_wait_seconds,
    ) as slot_index:
        log_event(
            "WATCHDOG_PROFILER_SLOT_ACQUIRED",
            ticker=args.ticker,
            slot=slot_index,
        ...
        try:
            ...
            raise RuntimeError(
                f"profiler timed out after {timeout_seconds:.1f}s"
            ) from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--profiler-timeout-seconds",
        type=float,
        default=None,
        help=(
            "Hard timeout for each profiler child. Defaults to sample-seconds "
            f"plus {DEFAULT_PROFILER_TIMEOUT_GRACE_SECONDS:.0f}s."
        ),
    )
    parser.add_argument(
        "--profiler-max-concurrency",
        type=int,
        default=DEFAULT_MAX_CONCURRENT_PROFILERS,
        help="Maximum profiler children allowed across all watchdog processes for this user.",
    )
    parser.add_argument(
        "--profiler-slot-wait-seconds",
        type=float,
        default=DEFAULT_PROFILER_SLOT_WAIT_SECONDS,
        help="Maximum time a watchdog waits for a shared profiler slot.",
    )
    parser.add_argument(
        "--profiler-slot-dir",
        default=str(DEFAULT_PROFILER_SLOT_DIR),
        help="Directory containing the shared advisory-lock profiler slots.",
    )
    parser.add_argument(
        "--profiler-stagger-max-seconds",
        type=float,
        default=DEFAULT_PROFILER_STAGGER_MAX_SECONDS,
        help="Maximum deterministic per-ticker delay before the first profiler cycle.",
    )
...
def main() -> int:
    ...
    stagger_seconds = deterministic_stagger_seconds(
        args.ticker,
        interval_seconds=args.interval_seconds,
        maximum_seconds=args.profiler_stagger_max_seconds,
    )
    ...
    log_event(
        ...
        profiler_timeout_seconds=f"{profiler_timeout_seconds(args):.1f}",
        profiler_max_concurrency=max(1, int(args.profiler_max_concurrency)),
        initial_stagger_seconds=f"{stagger_seconds:.3f}",
    )
